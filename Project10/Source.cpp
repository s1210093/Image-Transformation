...
int Ax, Ay, Bx, By, Cx, Cy, Dx, Dy;   //範囲を示す座標の行列
int Tr, Tg, Tb;   //色
Point2i pre_point;  //わからない
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	Mat in_img = imread("./PICTURE/78.jpg"); //画像読み込み
	...
	for (int i = 1; i <= 1; i++){
		...
		*/
		...
		// in_img = undist(in_img) ; //　カメラのゆがみを取る(Gopro魚眼)

		//画像をリサイズ(大きすぎるとディスプレイに入りきらないため)
		resize(in_img, src_img, Size(src_img_cols, src_img_rows), CV_8UC3);

		if (i == 1) {
			namedWindow("getCoordinates"); //ウィンドウの名前設定
			imshow("getCoordinates", src_img);//入力用画像の表示
